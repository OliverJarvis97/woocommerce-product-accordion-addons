...
final class G2_WC_Product_Accordion_Addons {
	...
	public function check_for_updates( $transient ) {
		...
		$current_version = isset( $transient->checked[ $plugin_file ] ) ? $transient->checked[ $plugin_file ] : '1.7.4';
	...
	private function price_rule( $parent_id, $addon_id, $variation_id = 0 ) { $config = $this->config( $parent_id, $addon_id ); $settings = $this->settings(); $rule = ! empty( $variation_id ) && ! empty( $config['variation_rules'][ $variation_id ] ) ? $config['variation_rules'][ $variation_id ] : $config; $type = $rule['price_type'] ?? 'inherit'; $value = 'inherit' === $type ? $settings['price_value'] : ( $rule['price_value'] ?? '' ); $type = 'inherit' === $type ? $settings['price_type'] : $type; return array( 'type' => in_array( $type, array( 'fixed', 'percent' ), true ) ? $type : 'none', 'value' => max( 0, (float) $value ) ); }
	...
	private function render_admin_config( $parent_id, $addon ) {
		...
		echo '<details class="g2-wc-admin-addon" style="margin:10px 0;border:1px solid #ddd;background:#fff"><summary style="padding:12px;cursor:pointer;font-weight:600">' . esc_html( $addon->get_name() ) . '</summary><div style="padding:0 12px 12px">';
		...
		if ( $addon->is_type( 'variable' ) ) { $selected = array_map( 'absint', (array) ( $config['variation_ids'] ?? array() ) ); echo '<p class="form-field"><label>' . esc_html__( 'Allowed variations', 'wc-product-accordion-addons' ) . '</label><select multiple style="min-width:300px" name="' . esc_attr( $prefix ) . '[variation_ids][]">'; foreach ( $addon->get_children() as $variation_id ) { $variation = wc_get_product( $variation_id ); if ( $variation ) { echo '<option value="' . esc_attr( $variation_id ) . '"' . selected( in_array( absint( $variation_id ), $selected, true ), true, false ) . '>' . esc_html( $this->variation_label( $variation, $addon ) ) . '</option>'; } } echo '</select> ' . wc_help_tip( __( 'Leave empty to allow all. With one allowed variation, no dropdown is shown.', 'wc-product-accordion-addons' ) ) . '</p>';
			$rules = isset( $config['variation_rules'] ) && is_array( $config['variation_rules'] ) ? $config['variation_rules'] : array(); echo '<p><strong>' . esc_html__( 'Variation price overrides', 'wc-product-accordion-addons' ) . '</strong><br><span class="description">' . esc_html__( 'Optional. These override the add-on and global rules for that specific variation.', 'wc-product-accordion-addons' ) . '</span></p>';
			foreach ( $addon->get_children() as $variation_id ) { $variation = wc_get_product( $variation_id ); if ( ! $variation ) { continue; } $rule = isset( $rules[ $variation_id ] ) && is_array( $rules[ $variation_id ] ) ? $rules[ $variation_id ] : array(); $rule_type = $rule['price_type'] ?? 'inherit'; echo '<p class="form-field"><label>' . esc_html( $this->variation_label( $variation, $addon ) ) . '</label><select name="' . esc_attr( $prefix ) . '[variation_rules][' . esc_attr( $variation_id ) . '][price_type]"><option value="inherit"' . selected( $rule_type, 'inherit', false ) . '>' . esc_html__( 'Use add-on/global rule', 'wc-product-accordion-addons' ) . '</option><option value="none"' . selected( $rule_type, 'none', false ) . '>' . esc_html__( 'Use variation price', 'wc-product-accordion-addons' ) . '</option><option value="fixed"' . selected( $rule_type, 'fixed', false ) . '>' . esc_html__( 'Set fixed price', 'wc-product-accordion-addons' ) . '</option><option value="percent"' . selected( $rule_type, 'percent', false ) . '>' . esc_html__( 'Percentage discount', 'wc-product-accordion-addons' ) . '</option></select> <input type="number" min="0" step="0.01" class="short" name="' . esc_attr( $prefix ) . '[variation_rules][' . esc_attr( $variation_id ) . '][price_value]" value="' . esc_attr( $rule['price_value'] ?? '' ) . '" placeholder="' . esc_attr__( 'Amount', 'wc-product-accordion-addons' ) . '"></p>'; }
		}
		echo '</div></details>';
	}
	...
	public function save_admin_field( $post_id ) {
		...
		foreach ( $ids as $addon_id ) { $item = isset( $raw_configs[ $addon_id ] ) && is_array( $raw_configs[ $addon_id ] ) ? $raw_configs[ $addon_id ] : array(); $type = isset( $item['price_type'] ) ? sanitize_key( $item['price_type'] ) : 'inherit'; $type = in_array( $type, array( 'inherit', 'none', 'fixed', 'percent' ), true ) ? $type : 'inherit'; $value = isset( $item['price_value'] ) ? (float) wc_format_decimal( $item['price_value'] ) : 0; $value = 'percent' === $type ? min( 100, max( 0, $value ) ) : max( 0, $value ); $variation_ids = array_values( array_unique( array_filter( array_map( 'absint', (array) ( $item['variation_ids'] ?? array() ) ) ) ) ); $addon = wc_get_product( $addon_id ); $variation_rules = array(); if ( ! $addon || ! $addon->is_type( 'variable' ) ) { $variation_ids = array(); } else { $children = array_map( 'absint', $addon->get_children() ); $variation_ids = array_values( array_intersect( $variation_ids, $children ) ); foreach ( (array) ( $item['variation_rules'] ?? array() ) as $variation_id => $rule ) { $variation_id = absint( $variation_id ); if ( ! in_array( $variation_id, $children, true ) || ! is_array( $rule ) ) { continue; } $rule_type = isset( $rule['price_type'] ) ? sanitize_key( $rule['price_type'] ) : 'inherit'; $rule_type = in_array( $rule_type, array( 'inherit', 'none', 'fixed', 'percent' ), true ) ? $rule_type : 'inherit'; $rule_value = isset( $rule['price_value'] ) ? (float) wc_format_decimal( $rule['price_value'] ) : 0; $variation_rules[ $variation_id ] = array( 'price_type' => $rule_type, 'price_value' => 'percent' === $rule_type ? min( 100, max( 0, $rule_value ) ) : max( 0, $rule_value ) ); } } $configs[ $addon_id ] = array( 'name' => isset( $item['name'] ) ? sanitize_text_field( $item['name'] ) : '', 'description' => isset( $item['description'] ) ? sanitize_textarea_field( $item['description'] ) : '', 'allow_quantity' => ! empty( $item['allow_quantity'] ) ? 'yes' : 'no', 'price_type' => $type, 'price_value' => $value, 'variation_ids' => $variation_ids, 'variation_rules' => $variation_rules ); }
	...
	public function render_addons() {
		...
		foreach ( $addons as $addon_id ) {
			...
			$rule = $this->price_rule( $product->get_id(), $addon_id, $single_variation ? $single_variation->get_id() : 0 );
			...
			if ( $addon->is_type( 'variable' ) && ! $single_variation ) {
				...
				foreach ( $variations as $variation_product ) { $variation_rule = $this->price_rule( $product->get_id(), $addon_id, $variation_product->get_id() ); $variation_price_html = 'none' === $variation_rule['type'] ? $variation_product->get_price_html() : wc_price( $this->adjusted_price( (float) $variation_product->get_price(), $variation_rule ) ); echo '<option value="' . esc_attr( $variation_product->get_id() ) . '">' . esc_html( $this->variation_label( $variation_product, $addon ) . ' — ' . wp_strip_all_tags( $variation_price_html ) ) . '</option>'; }
				echo '</select></span>';
			}
	...
	public function store_addons_in_cart_item( $cart_item_data, $product_id, $variation_id ) {
		...
		if ( $addons ) {
			$items = array(); foreach ( $addons as $addon_id ) { $config = $this->config( $product_id, $addon_id ); $variation = $this->requested_addon_variation( $product_id, $addon_id ); $variation_id = $variation ? $variation->get_id() : 0; $items[] = array( 'id' => $addon_id, 'variation_id' => $variation_id, 'quantity' => ! empty( $config['allow_quantity'] ) && 'yes' === $config['allow_quantity'] ? max( 1, absint( $_POST['g2_wc_addon_quantities'][ $addon_id ] ?? 1 ) ) : 1, 'rule' => $this->price_rule( $product_id, $addon_id, $variation_id ), 'allow_quantity' => ! empty( $config['allow_quantity'] ) && 'yes' === $config['allow_quantity'] ); }
