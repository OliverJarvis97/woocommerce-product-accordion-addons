...
final class G2_WC_Product_Accordion_Addons {
	...
	public function check_for_updates( $transient ) {
		...
		$current_version = isset( $transient->checked[ $plugin_file ] ) ? $transient->checked[ $plugin_file ] : '1.7.2';
	...
	public function enqueue_assets() {
		...
		wp_register_style( 'g2-wc-product-accordion-addons', false, array(), '1.7.2' );
		...
		wp_register_script( 'g2-wc-product-accordion-addons', false, array(), '1.7.2', true );
	...
	public function render_addons() {
		...
		foreach ( $addons as $addon_id ) {
			...
			if ( $addon->is_type( 'variable' ) && ! $single_variation ) {
				...
				foreach ( $variations as $variation_product ) { $variation_rule = $this->price_rule( $product->get_id(), $addon_id, $variation_product->get_id() ); echo '<option value="' . esc_attr( $variation_product->get_id() ) . '">' . esc_html( $this->variation_label( $variation_product, $addon ) . ' — ' . wp_strip_all_tags( 'none' === $variation_rule['type'] ? $variation_product->get_price_html() : wc_price( $this->adjusted_price( (float) $variation_product->get_price(), $variation_rule ) ) ) . '</option>'; }
				echo '</select></span>';
			}
